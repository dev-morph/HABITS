                    ...
                    <div v-for="(row, index) in calendar" :key="index" class="calendar__row">
                        <div
                            ...
                            :class="{
                                ...
                                today:
                                    cell.month === today.get('month') + 1 &&
                                    cell.date === today.get('date'),
                                selected:
                                    cell.month === selected.month && cell.date === selected.date
                            }"
...
const { selected, selectHandler } = toRefs(props)
...
function generateFirstRow() {
    ...
    while (dayOfFirstDay - start > 0) {
        ...
        if (targetDate.value.prevMonth === 12) {
            row.push({
                year: targetDate.value.prevYear,
                month: targetDate.value.prevMonth,
                date: date
            })
        } else {
    ...
    if (date >= lastDayOfThisMonth && row.length < 7) {
        ...
        while (row.length < 7) {
            ...
            if (targetDate.value.nextMonth === 1) {
                row.push({
                    year: targetDate.value.nextYear,
                    month: targetDate.value.nextMonth,
                    date: date
                })
            } else {
                row.push({
                    year: targetDate.value.year,
                    month: targetDate.value.nextMonth,
                    date: date
                })
            }
